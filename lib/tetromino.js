...
class TetrominoModel {
    constructor() {
        ...
        this.engine.highScoreLines = atom.config.get(CONF_HIGHSCORE_LINES);
        this.engine.on(engine.EVENT_HIGHSCORE, () => {
            const newHighScoreLines = this.engine.highScoreLines;
            const lastHighScoreLines = atom.config.get(CONF_HIGHSCORE_LINES) || 0;
            if (newHighScoreLines > lastHighScoreLines) {
                atom.config.set(CONF_HIGHSCORE_LINES, newHighScoreLines);
            }
        });
    }
