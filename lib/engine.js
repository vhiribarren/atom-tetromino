...
function convertLevelToTimer(level) {
    const level0 = 0.8 * 1000;
    const level9 = 0.2 * 1000;
    if (level < 10) {
        const m = (level9-level0)/(9-0);
        return  level*m + level0;
    }
    else {
        return (9*level9)/(level);
    }
}
...
function gameTick() {
    ...
    drawGameState();
    setTimeout(gameTick, convertLevelToTimer(gameState.level));
}
