...
function newIncomingShape() {
    ...
    const shapeWidth = gameState.shape.getShape()[0].length;
    gameState.shapePos.x = Math.floor((BOARD_WIDTH-shapeWidth)/2);
...
function processLines(yTop, yBottom) {
...
function gameTick() {
    if (gameState.shape === null) {
        ...
        if (checkCollision(gameState.shapePos.x, gameState.shapePos.y+1)) {
            const yTop = gameState.shapePos.y;
            const yBottom = gameState.shape.getShape().length -1 + yTop;
            copyShapeToBoard();
            processLines(yTop, yBottom);
            newIncomingShape();
        }
        else {
            fallBlock();
